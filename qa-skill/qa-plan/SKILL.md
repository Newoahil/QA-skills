...
First, independently inspect the actual available Diff rather than relying on a summary, along with existing test coverage, test configuration, and project-provided validation commands. Record the inspected Diff context and available coverage in the report before planning.

Before `Objective and Scope`, record a named `Change Intake` with the exact fields `Observed Facts`, `Inferred Intent` with confidence and basis, `Authoritative Acceptance Criteria` with source/owner, and `Unresolved Questions`. Inferred intent must not become expected behavior without authoritative support.
