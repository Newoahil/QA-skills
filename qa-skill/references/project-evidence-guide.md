...
A missing module-configured runner inside the isolated workspace is not automatically the end of verification for that module. If the module's own language runtime is already present, and the already-existing, unmodified module source can be exercised directly and safely inside the isolated workspace — no install, no dependency mutation, no network, no generated or fake test file — that direct invocation is itself a safe already-available local existing check, not a new tool being forced onto the module. Only record `BLOCKED` for a module verification item after actually considering whether such a direct, safe, read-only, in-workspace path exists.
