...
这一分支的建设重点，是把原有 `qa-skill` 从日常开发 QA 路径中拆出来，先沉淀成适合值守闭环场景的独立能力。当前项目焦点是稳定四角色 capability：`qa`、`qa-cr`、`qa-api`、`qa-e2e`。与旧的日常 qa-check / main 路线不同，本分支不使用 `using-qa` 作为产品主入口，而是围绕这四个角色建立可裁判、可短路、可追溯的闭环，其中 `qa` 始终是唯一 verdict owner。

`qa-orchestrator` 分支建立了这套能力基线，当前阶段先稳定独立 agent capability layer；外部产品接线不在本文档范围内。
...
值守调用方
  ...
  -> CR passes -> qa-api for non-browser HTTP/API claims when required
  -> CR passes -> qa-e2e for browser / user-flow claims when required
  -> qa reconciles raw evidence
  -> exactly one Overall Status
```

这个流程的关键不是“多一个 QA agent”，而是先做 focused start，再以 CR 作为代码变更的默认和强制闸门；只有在不用运行时观察就无法建立 oracle、trigger 或有界 CR scope 时，才允许先拿一次最小 runtime diagnostic。这个 diagnostic 只用于定界，不替代 CR，也不豁免后续 required verification。CR 之后，再按 claim 类型分流到 `qa-api` 或 `qa-e2e`：非浏览器 HTTP/API 证据走 `qa-api`，浏览器/用户流证据走 `qa-e2e`。与此同时，动态证据收集和最终 verdict ownership 仍然分开，并通过结构化协议确保 subagent 输出能重新收口回 `qa`。
...
- 把 `qa-skill/agents/qa.md`、`qa-skill/agents/qa-cr.md`、`qa-skill/agents/qa-api.md`、`qa-skill/agents/qa-e2e.md` 复制到 `~/.config/opencode/agents/`
...
- `references/` 存放 `qa`、`qa-api`、`qa-e2e` 的协作约束，以及 runner / adapter 的安全不变式说明。
- `scripts/e2e-runner.mjs` 是通用、工具无关、单次有界的 e2e 执行器。
- `qa.md` 是唯一 verdict owner。
- `qa-cr.md` 是 P0 quality-oriented CR evidence subagent。
- `qa-api.md` 是 post-CR non-browser HTTP / API evidence subagent。
...
- **CR-first 质量短路机制**：代码变更默认必须进入 CR gate。小 diff 可由 `qa` 直接做 inline CR-like review；复杂、高风险、上下文重的 diff 交给 `qa-cr`。只有在建立 oracle、trigger 或 scope 必需时，才允许一次最小有界 pre-CR runtime diagnostic；一旦出现已验证的 load-bearing CR FAIL，就直接停止后续重型证据，不再进入后续 `qa-api` / `qa-e2e` 重验证。
...
- **post-CR API 证据能力**：`qa-api` 只在真实 non-browser HTTP / API 行为成为 load-bearing 问题时启用，优先使用本地可控环境与已有调用路径；外部环境、真实身份、变更性操作都需要显式批准与边界约束。它不负责传输层调试，也不把 shell/budget 边界误表述为“机械仓库只读沙箱”。

- **post-CR e2e 证据能力**：`qa-e2e` 只在真实 UI / browser / end-to-end 行为成为 load-bearing 问题时启用，并适配项目已有 Cypress、Playwright、Selenium、WebdriverIO 或自定义脚本，而不是按工具拆 agent。真实 Playwright 基准消费的是调用方提供的现有模块路径，不为目标项目新增或安装 Playwright 应用依赖；模块不存在时应及早失败并如实回报环境缺口。浏览器资产默认不自动安装，只有在明确 assignment 授权且确有需要时才可安装，并且永远不安装目标应用依赖。

- **受控 e2e-runner 执行**：`e2e-runner` 提供一次有界的 `start -> ready -> test -> cleanup` 执行模型，约束总预算、端口占用、进程回收和机器证据输出。

- **environment-needed handoff 能力**：当 required evidence 因服务、seed、secret、browser asset、test identity 或 runtime 受限而无法获得时，`qa` 输出 `BLOCKED` 或残余风险，并明确给出下一步所需环境，而不是假 PASS。

## 解决的痛点

- builder / fixer 自审天然不客观，容易把“我觉得修好了”误当成可裁判的 QA 结论。

- 旧路径里常见“CR 还没过就先跑重型测试”，一旦代码层面就存在 load-bearing 问题，后续 API / browser / build / e2e 成本被白白消耗。
...
- 非浏览器 API / integration 证据采集：只有当代码证据不足以说明 HTTP / API 行为时，才派 `qa-api`。
...
- 未来定时任务 / CI gate：当前能力层已经具备结构化输出、bounded runtime 和短路机制，可被上游 runtime 复用，但这不等于当前项目已把完整生产 watchdog loop 纳入范围。

## 通用能力

- **focused evidence-driven start**：先确认目标、实际 change surface、oracle/commitments 和预算，不从“全项目看看有没有问题”起手；必要时允许一次最小有界 runtime diagnostic 来建立 oracle、trigger 或 scope。
...
- **inline review + subagent split**：小问题可在 `qa` 内联完成，大问题再分派给 `qa-cr`、`qa-api` 或 `qa-e2e`，避免为拆分而拆分。
...
- **严格裁判边界**：`qa` 与 `qa-cr` 显式无 shell；`qa-api` 与 `qa-e2e` 也不得编辑产品代码、测试、文档或配置。它们的 bash 权限边界是受控执行策略，不应被误解成机械的 repository-read-only 沙箱；QA 仍不负责修复、不负责 ship decision。
...
|---|---|---|---|---|
|bounded CR gate|代码级风险排查|常见是先混合看 diff、跑测试、再回头解释失败|先 `qa` focused start，再进 mandatory CR gate；必要时仅先做最小 diagnostic，再由 `qa-cr` / inline CR 完成 CR|load-bearing CR FAIL 可在进入 `qa-api` / `qa-e2e` 前直接短路|
...
|环境阻塞处理|服务 / seed / browser asset / identity 缺失|容易反复重试或误判为失败|明确 `BLOCKED` / environment-needed handoff|减少无效等待和误连未知旧服务|

确定性可以确认的只有：

- 单个 bounded `e2e-runner` 默认总预算最多 10 分钟，并为 cleanup 预留预算。
- 已验证的 CR fail 可以短路后续 API / browser / e2e / 重型证据。
...
QA Skill 当前仍是一个独立 agent capability layer。它已经具备基本的值守闭环裁判结构，但还有明确边界：

- `qa`、`qa-cr`、`qa-api`、`qa-e2e` 四角色当前都属于正式能力面；`qa` 始终是唯一 verdict owner。

- 没有泛化 `qa-facet`；当前不再继续扩展开放式 shard worker 路径。

- `qa` 与 `qa-cr` 显式没有 shell；动态证据依赖 `qa-api`、`qa-e2e` 或调用方已提供的运行结果。

- QA 不自动修复、不写测试、不做 ship / release decision。

- `qa-api` 默认 local-first；访问外部环境、使用真实测试身份、执行变更性 mutation 都需要显式批准。mutation 还必须事先界定数据范围、预算、cleanup / rollback、idempotency 和 redaction；4xx / 5xx 可以作为观察性证据，但传输层故障排查属于 `BLOCKED`，不在 `qa-api` 执行范围内。

- 不安装应用依赖；`qa-e2e` 默认也不自动安装浏览器资产，缺失时先返回 `BLOCKED` 与安装建议。只有在明确 assignment 授权且确有需要时，才可安装浏览器资产，并且永远不安装目标应用依赖。
...
- orchestrator 当前确定性数字为 61 total、49 pass、12 real-model opt-in skip、0 fail。

- 当前真实 paired scenario 共 12 个，均为成对验证且带预期 outcome；其中真实 model eval 仍然是 opt-in、环境相关，不承诺所有环境都可直接连续复现。

- 当前 12 个真实 paired scenario 均已有预期 outcome；其中 autonomous API rerun 曾因 provider timeout 需要一次重试后完成，这也说明真实 model eval 仍受环境与外部提供方状态影响。
...
本轮完成的是 format-debt reduction cycle：worker 侧原来的长模板已收敛为紧凑 contract，pre-CR 只保留建立 oracle / trigger / scope 所需的最小有界 diagnostic，而正式 API / e2e 证据仍默认位于 post-CR；此前把 pre-CR diagnostic 与 post-CR 主验证混写的表述已经修正。CR 结论如果一时 inconclusive，可以继续收集额外证据，但只要存在 material gap，就不能直接关闭 PASS。

当前范围仍然是 agent capability：

- 四角色 capability 已经成形：`qa` 负责 verdict，`qa-cr` 负责代码审查证据，`qa-api` 负责 non-browser HTTP / API 证据，`qa-e2e` 负责浏览器 / 用户流证据。
- 不再沿用过时的 schema debt / fail-closed debt 表述；当前重点已转向扩大真实样本、稳固 contract、以及保持 capability 边界清晰。

后续方向：

1. **扩大真实 benchmark 与 CR / API / e2e 样本**
   继续为 `qa-cr`、`qa-api`、`qa-e2e` 补充更系统的真实 diff / issue / runtime 样本，覆盖 stop_and_fail、动态扩展、可信静态 FAIL、观察性 4xx/5xx、以及 contract 宽松后仍可稳定收口的场景。

2. **继续增强 e2e 异常终止与 cleanup 鲁棒性**
   持续提升 abnormal termination、孤儿进程识别、cleanup 失败、端口释放异常时的恢复与证据表达能力。

3. **稳定文档、安装与报告消费约束**
   继续收敛安装路径、skill 复制说明、`qa-api.md` / `qa-e2e.md` 分工、runner 调用方式、双语报告示例与 evidence 写法，减少误装、误写和误判。

4. **继续稳固 `qa-api` 的本地优先与安全边界**
   持续补强 external approval、test identity、mutation budget / cleanup / idempotency、redaction、4xx/5xx observational 与 transport BLOCKED 的文档和样本，同时仍保持 verdict ownership 在 `qa`。
