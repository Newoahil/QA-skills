...
export function processPlanBuilder({ issue, repoDir, qaRuntimeDir = repoDir, guardianDir = null, dossier, issueData = null, timeoutMs = 600000, opencodeClient, state = null, round = 1, memoryContext = null, fallbackModels = [], model = undefined, deadlineMs = 0, spawnImpl = spawn, previousPlanErrors = [], attempt = 1 }) {
  const promptLines = [
    ...
    'test_commands MUST be executable argv arrays. Valid Node forms are node --test <scoped-repo-test-path>... or the exact allowlisted project regression script frontend/apps/alipay-miniapp/scripts/test-category-builder-runtime.js. Valid Python forms are python -m pytest <scoped .py target> [safe flags], py -m pytest <scoped .py target> [safe flags], python -m unittest <scoped .py target or dotted tests.* target> [safe flags], py -m unittest <scoped .py target or dotted tests.* target> [safe flags], or python/py -m unittest discover -s tests|test|__tests__ -p test_*.py|*_test.py [safe flags]. Do not use node --test for .py files. Do not return arbitrary Python modules, scripts, arbitrary Python scripts, arbitrary module names, config/plugin injection, shell strings, wrappers, traversal, network, git, install, or unknown executables.',
