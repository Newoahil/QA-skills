...
- [decision-832596314b274789ab3bf5f8e354d69d] #qa-guardian #testing #recovery #lease — 重置单个 Guardian issue 只需备份并删除权威 `.qa/guardian/<n>.json` 与 `<n>/`；lease 存在 `<n>.json` 里会随之清除，`.scheduler.lock` 与旧 `/guardian` 评论无需手动删，`config.json`/其它 issue/`watch-state.json`/`.sybermem` 一律不动；重启后 `in-progress-fresh-lease` 只是旧 lease 未过期而非卡死。 (2026-08-25)
