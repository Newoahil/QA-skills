...
- [decision-832596314b274789ab3bf5f8e354d69d] #qa-guardian #testing #recovery #lease — 重置单个 Guardian issue 只需备份并删除权威 `.qa/guardian/<n>.json` 与 `<n>/`；lease 存在 `<n>.json` 里会随之清除，旧 `/guardian` 评论无需手动删，`config.json`/其它 issue/`watch-state.json`/`.sybermem` 一律不动；重启后 `in-progress-fresh-lease` 只是旧 lease 未过期而非卡死。补充：`Ctrl+C` 正常退出会自动释放 `.scheduler.lock`，但强杀进程会残留死锁——reset/重启前若 `lock_exists=True`，先核对锁里的 owner pid 是否存活（`Get-Process -Id <pid>`），死了才安全删锁，活着说明 scheduler 还在跑要先停。 (2026-08-25)
...
- recovery: bug-209c918349934460abcb4741bece9f0d, bug-432ac9e0feec425a91229512c7603ac9, change-55e41d2949a84e69a7525fbf771f2201, change-a43b7803dba74e9bae48e0bed222011c, decision-f177ceabeded4a5193afa0f2b58b6c13
...
- testing: decision-f177ceabeded4a5193afa0f2b58b6c13
