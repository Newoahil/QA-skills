...
test('scheduler-start.ps1 retries GitHub preflight and preserves real diagnostics', () => {
  const text = readFileSync('tools/guardian/scheduler-start.ps1', 'utf8');
  ...
  assert.match(text, /function Invoke-GhPreflight/);
  assert.match(text, /\[int\]\$MaxAttempts = 3/);
  assert.match(text, /Start-Sleep -Seconds \$attempt/);
  assert.match(ghBlock, /Invoke-GhPreflight @\('api', 'user', '--jq', '\.login'\)/);
  assert.match(ghBlock, /Invoke-GhPreflight @\('repo', 'view', \$targetGithub\)/);
  assert.match(ghBlock, /GitHub 认证预检失败/);
  assert.match(ghBlock, /\$ghAuth\.output/);
  assert.doesNotMatch(ghBlock, /gh 尚未登录，请先执行 gh auth login/);
});
...
test('scheduler launcher allows terminal handed-back dirty plan without blocking startup', () => {
  const text = readFileSync('tools/guardian/scheduler-start.ps1', 'utf8');
  const controlFunction = text.slice(text.indexOf('function Ensure-ControlWorktree'), text.indexOf('function Ensure-QaSnapshot'));
  assert.match(controlFunction, /terminalHandedBackDirty = \[string\]\$state\.state -eq 'HANDED_BACK' -and\s+\[string\]\$state\.handed_back_reason -in @\('fix-rounds-exceeded', 'reject', 'stalled', 'supervisor-run-failed'\)/);
  assert.match(controlFunction, /-and -not \$recoverablePlanScopeHandback/);
